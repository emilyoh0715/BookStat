    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: `책 표지 이미지를 분석해서 책 제목과 저자를 추출하세요.

단계별로 수행하세요:
1. 표지에서 가장 크고 눈에 띄게 쓰인 텍스트가 무엇인지 확인하세요. 그것이 주 제목입니다.
2. 한글(가~힣)과 영어가 모두 있다면, 더 크게 표시된 언어의 텍스트를 제목으로 선택하세요.
3. 영어가 더 작게 쓰여 있다면 그것은 부제이므로 무시하세요.
4. 선택한 텍스트를 절대 번역하거나 다른 언어로 변환하지 마세요. 보이는 그대로 출력하세요.
5. 출판사명, 시리즈명, 추천사 등은 제외하세요.

다음 JSON 형식으로만 응답하세요 (설명 없이):
{"title": "가장 크게 쓰인 주 제목 원문", "author": "저자명 원문"}
확인할 수 없는 항목은 ""로 두세요.`,
          },
        ],
      }],
