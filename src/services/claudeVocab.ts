    ...
    body: JSON.stringify({
      ...
      messages: [{
        ...
        content: [
          ...
          {
            type: 'text',
            text: `표지 이미지에서 책 제목과 저자 이름을 OCR로 읽어주세요.

규칙:
- 한글(ㄱ~ㅎ, 가~힣)이 보이면 반드시 한글 그대로 출력하세요. 영어로 번역하거나 로마자로 변환하는 것은 절대 금지입니다.
- 영어만 있는 경우 영어 그대로 출력하세요.
- 제목은 표지에서 가장 크게 쓰인 텍스트입니다.
- 부제나 출판사명은 제외하고 주 제목과 저자만 추출하세요.

다음 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{"title": "원문 텍스트", "author": "원문 텍스트"}
확인할 수 없는 항목은 ""로 두세요.`,
          },
        ],
      }],
